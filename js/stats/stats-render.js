...
function renderKeyMetrics(data) {
    ...
    grid.innerHTML = `
        ...
        <div class="stat-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Skandaler</p>
                    <p class="text-4xl font-bold text-gray-900 dark:text-white">${totalScandals}</p>
                </div>
        ...
        <div class="stat-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold text-gray-900 dark:text-white">${totalBrokenPromises}</p>
                </div>
...
function renderPartyDistribution(data) {
    ...
    Object.keys(byParty).sort((a,b) => byParty[b].count - byParty[a].count).forEach(party => {
        const pct = Math.round((byParty[party].count / data.length) * 100);
        html += `
            <div class="flex items-center gap-4">
                <div class="w-6 h-6 rounded-lg flex-shrink-0" style="background-color: ${byParty[party].color}"></div>
                <div class="flex-1">
                    <div class="flex justify-between text-sm mb-1">
                        <span class="font-medium">${party}</span>
                        <span>${byParty[party].count} (${pct}%)</span>
                    </div>
        ...
        html += `
            <div onclick="showPoliticianModal('${p.slug}')" class="flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-2xl cursor-pointer transition-colors">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-xl flex items-center justify-center text-lg" style="background-color: ${p.color}20; color: ${p.color}">
                        ${p.partyShort}
                    </div>
                    <div>
                        <p class="font-medium">${p.name}</p>
                        <p class="text-xs text-gray-500">${p.party}</p>
                    </div>
                </div>
